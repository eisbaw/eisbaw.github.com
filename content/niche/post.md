...
**This blog runs on a static-site engine that treats Nix as the build system, not just the package manager.** It is called niche. A small Rust binary does the text work; a Nix flake orchestrates everything else and caches the result.
...
Nix hands you caching for free. Each compiled post is a store path. Nothing changed means nothing rebuilds. The public surface is one function: `niche.lib.mkSite { pkgs; contentDir; siteConfig; themeDir; }` returns the built site as a derivation.
...
- **`render`** — Markdown, RST, HTML, or plain text to an HTML fragment. Syntax highlighting via syntect. Wiki-links left as placeholders.
- **`link`** — resolve those placeholders against the JSON registry. Warn on anything broken.
- **`compose`** — wrap fragments in Tera templates, build the aggregate pages, copy static assets.

Markdown goes through comrak (CommonMark + GFM). RST shells out to `rst2html5` from docutils. HTML passes through. Plain text gets wrapped in `<pre>`.
